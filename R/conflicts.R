#' Conflicts between the tinytools and other packages
#'
#' This function lists all the conflicts between packages in the tinytools
...
tinytools_conflicts <- function() {
  ...
  tidy_names <- paste0("package:", tinytools_packages())
  conflicts <- purrr::keep(conflicts, ~ any(.x %in% tidy_names))
  
  conflict_funs <- purrr::imap(conflicts, confirm_conflict)
  conflict_funs <- purrr::compact(conflict_funs)
  
  structure(conflict_funs, class = "tinytools_conflicts")
}
...
#' @export
print.tinytools_conflicts <- function(x, ..., startup = FALSE) {
  cli::cat_line(tinytools_conflict_message(x))
}
